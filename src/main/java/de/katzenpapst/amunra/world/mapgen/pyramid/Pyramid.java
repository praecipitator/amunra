...
public class Pyramid extends BaseStructureStart
{
    ...
    public void setMainRoom(PyramidRoom room) {
        ...
        mainEntranceBB.minX = innerRoomBB.getCenter().getX()-1;
        mainEntranceBB.maxX = innerRoomBB.getCenter().getX()+1;
        mainEntranceBB.minZ = innerRoomBB.maxZ+1;
        mainEntranceBB.maxZ = innerRoomBB.maxZ+4;
    ...
    public boolean generateChunk(int chunkX, int chunkZ, ChunkPrimer primer) {
        ...
        for(int x = startX; x <= stopX; x++) {
            for(int z = startZ; z <= stopZ; z++) {
                ...
                for(int y=highestGroundBlock-1;y<groundLevel; y++) {
                    //padding
                    placeBlockRel2BB(primer, chunkX, chunkZ, new BlockPos(x, y, z), fillMaterial);
                }
                // floor
                placeBlockRel2BB(primer ,chunkX, chunkZ, new BlockPos(x, groundLevel, z), floorMaterial);
                ...
                for(int y = 0; y <= radius; y++) {
                    ...
                    if((x >= startX+y && x <= stopX-y) && (z >= startZ+y && z <= stopZ-y)) {
                        if((z == startZ+y || z == stopZ-y) || (x == startX+y || x == stopX-y)) {
                            // wall
                            placeBlockRel2BB(primer, chunkX, chunkZ, new BlockPos(x, groundLevel+y+1, z), wallMaterial);
                        } else {
                            // inner
                            placeBlockRel2BB(primer, chunkX, chunkZ, new BlockPos(x, groundLevel+y+1, z), fillMaterial);
                        }
                    }
                    if(y >= 5 && y <= 13) {
                        ...
                        if(x >= xCenter-4 && x <= xCenter+4) {
                            ...
                            if(x >= xCenter-1 && x <= xCenter+1 &&
                                ...
                                if(y == 5) {
                                    placeBlockRel2BB(primer, chunkX, chunkZ, new BlockPos(x, groundLevel+y+1, z), floorMaterial);
                                } else  {
                                    placeBlockRel2BB(primer, chunkX, chunkZ, new BlockPos(x, groundLevel+y+1, z), Blocks.air, 0);
                                }
                            }
                        }
        ...
        generateSmallRooms(chunkBB, primer);
    ...
    protected StructureBoundingBox getRoomEntranceBox(int position, StructureBoundingBox roomBox) {
        ...
        case 0: //up aka +z
            ...
            // hmm this is for odd
            doorBB.minX = roomBox.getCenter().getX()-1;
            doorBB.maxX = roomBox.getCenter().getX()+1;
            break;
        case 1: // right aka +x
            doorBB.minX = roomBox.maxX+1;
            doorBB.maxX = doorBB.minX+1;

            doorBB.minZ = roomBox.getCenter().getZ()-1;
            doorBB.maxZ = roomBox.getCenter().getZ()+1;
            break;
        case 2: //down aka -z
    ...
    protected void generateSmallRooms(StructureBoundingBox chunkBB, ChunkPrimer primer) {
        ...
        for(PyramidRoom r: roomList) {
            if(r.getStructureBoundingBox().intersectsWith(chunkBB)) {
                r.generateChunk(chunkX, chunkZ, primer);
            }
        }


        if(centralRoom.getStructureBoundingBox().intersectsWith(chunkBB)) {
            centralRoom.generateChunk(chunkX, chunkZ, primer);
        }
