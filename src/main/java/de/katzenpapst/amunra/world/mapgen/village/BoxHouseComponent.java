...
public class BoxHouseComponent extends GridVillageComponent {
    ...
    public boolean generateChunk(int chunkX, int chunkZ, ChunkPrimer primer) {
        ...
        for (int x = startX; x <= stopX; x++) {
            for (int z = startZ; z <= stopZ; z++) {
                ...
                if (x == xCenter && z == zCenter) {
                    ...
                    /*
                     * EntityCreature villager = new
                     * EntityRobotVillager(this.parent.getWorld());
                     * villager.onSpawnWithEgg(null);// NO IDEA int xOffset =
                     * getXWithOffset(x, z); //y = getYWithOffset(y); int
                     * zOffset = getZWithOffset(x, z);
                     * this.parent.spawnLater(villager, xOffset, groundLevel,
                     * zOffset);
                     */
                }
                ...
                for (int y = 0; y < houseHeight - 1; y++) {
                    ...
                    if (x == startX || x == stopX || z == startZ || z == stopZ) {
                        ...
                        this.shouldGenerateWindowHere(x, y, z, xCenter, startX, stopX, startZ, stopZ)) {
                            ...
                            placeBlockRel2BB(primer, chunkX, chunkZ, curPos, mat);
                        }
                ...
                // finally, roof
                placeBlockRel2BB(primer, chunkX, chunkZ, new BlockPos(x, groundLevel + houseHeight - 1, z), mat);
        ...
        if (highestGroundBlock != -1) {
            // groundLevel and groundLevel +1 should be free, and potentially
            // place
    ...
    private boolean
            shouldGenerateWindowHere(int x, int y, int z, int doorPos, int startX, int stopX, int startZ, int stopZ) {
